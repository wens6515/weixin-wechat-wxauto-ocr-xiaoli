# 小漓更新记录（2026-08-17）— v2.0.0「视觉版」

> **本次发布版本：v2.0.0「视觉版」**。核心变化：为适配微信 4.1.12+，弃用 wxauto4（UIA 通道），全面迁移到 OCR 视觉方案。
> 基线：GitHub 仓库 `wens6515/xiaoli-wechat-ai-bot` 的 `origin/main`（v1.0.1「初漓」，76 个提交）。
...
flowchart TD
    ...
    R --> S[定位 bot 最后回复 y: 窄列头像判据，没有则将整个消息区域视作对方消息，窗口 = y 在回复之后的对方消息，ocr判断是否有@消息]
    S -->|无@消息| A
    S -->|有@消息| F
    D --> E[定位 bot 最后回复 y: 窄列头像判据，没有则将整个消息区域视作对方消息]
    E --> F[窗口 = y 在回复之后的对方消息]
    F -->|窗口空| A
    F -->|有文字、无图片、无文件| G[OCR提取文字消息]
    F -->|可以有也可以没有文字、有图片或文件| H[sleep 10s防话没说完，重新截图 + 气泡色/气泡框/媒体矩形]
    G --> I[整合文字消息递交LLM判断是否为任务消息]
    I -->|不是任务消息| J[将文字消息发送给ai]
    I -->|是任务消息| K[投递给任务桥]
    H -->L[重新截图 + 气泡色/气泡框/媒体矩形，定位 bot 最后回复 y: 窄列头像判据，没有则将整个消息区域视作对方消息，窗口 = y 在回复之后的对方消息]
    L -->|有文字、有图片或文件| M[OCR提取文字，将文字交给LLM判断是否为任务消息]
    L -->|无文字、有图片、无文件| N[图片发送给多模态ai转成文字描述发送给ai]
    L -->|无文字、可以有图片也可以没有图片、有文件| O[文件流程: 回复收到 + 不停摆 + sender 关联] -->|收到sender消息| E
    M -->|不是任务消息| P[将图片发给多模态ai转成文字描述、文件提取成文字,然后整合发送给ai]
    M -->|是任务消息| Q[投递任务桥: 文字+图片截图+文件全投]
    J --> T[bot回复]
    K --> T[bot回复]
    N--> T[bot回复]
    P--> T[bot回复]
...
1. **版本号**：建议升 **v1.1.0**（架构级变化：通道迁移 + UI 重写；产品定位未变，不升 v2）。当前 `README.md`、`tools/小漓.iss` 仍是 v1.0.1，需同步。
2. **README 更新点**：
   - 功能一览：补充"视觉通道（新版微信 4.1.12 兼容）"与"12 套主题 + 壁纸库"
   - 前置条件：微信版本要求改为 4.1.12+（不再兼容 wxauto 旧协议）
   - 新增工具/标定说明小节（pick_ocr_region / fix_window）
3. **发布内容清单**（相对 GitHub 基线新增，需确认纳入版本库）：
   - 代码：`xiaoli_desktop/wx_backend/`、`xiaoli_desktop/xiaoli_app/ui/backdrop.py`、`ui/_icons.py`、`ui/assets/icons/`
   - 工具：`tools/` 全目录
   - 资源：`壁纸/`、`fonts/`、`小漓.ico`、`小漓.png`
   - 文档：本更新记录 + `docs/微信通道验证结论.md` + `docs/微信4本地数据破解与hook通道调研.md`
   - 未提交改动：`wx_backend/__init__.py`（wxauto 移除）、`wx_ocr_region.json`（真机标定值）、`wx_window.json`、`tests/test_send_method.py` 等（见 `git status`）
4. **注意**：两个旧交接文档（`交接文档.md`、`交接文档-Ghidra逆向.md`）已删除/被取代——`docs/微信通道验证结论.md` 承接其内容，开源发布时无需恢复。
