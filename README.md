...
> **v2.3.0「快档监听」** · 适配微信 PC **4.x** · 0.5s 恒定快档不间断监听 + OCR 两段化（首条消息 16~20 秒 → 约 6~9 秒）· API 指数退避与角色内友好报错 · 用量统计页 · 定时消息（聊天里一句话创建）· 微信窗口初始化自动定位

---

## ⬇️ 下载安装（桌面版）

**v2.3.0 完整桌面版安装包**（PySide6 图形界面 + 19 套主题 + 壁纸库 + 托盘常驻）：

👉 [点击下载 `xiaoli-setup-v2.3.0.exe`](https://github.com/wens6515/weixin-wechat-wxauto-ocr-xiaoli/releases/download/v2.3.0/xiaoli-setup-v2.3.0.exe)
...
flowchart TD
    ...
    L -->|④ 仅图片| I
    L -->|⑤ 纯文字| I
    I["vision-exp 单次调用<br/>看图读字 + 回复 + 任务/提醒判定"]
...
> 气泡/媒体分析无法区分视频、表情、图片，统一按图片处理；图片消息含文字时 10 秒等待防话没说完（此后重走的联合 OCR 未画出）；文件消息「回复收到 + 不停摆」，该发送者后续文字指令自动关联待处理文件。API 最终失败自动重试（429/5xx 指数退避 + 墙钟预算），重试耗尽发角色内友好提示，绝不把错误码原样发给好友。会话身份以标题区读取为准（列表条带名只作锚点），memory 键再剥引号与空格归一化；处理失败的会话 8 秒退避，防滞留红圈反复 OCR；微信被最小化时视觉通道是盲的，哨兵会自动恢复窗口。
