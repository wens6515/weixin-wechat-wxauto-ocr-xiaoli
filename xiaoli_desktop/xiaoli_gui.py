...
def main():
    ...
    from xiaoli_app.ui import build_qss, default_wallpaper_path, resolve_theme
    _theme = ctx.cfg.get("theme", "tokyonight")
    # 跟随系统深浅色：勾选时按系统深浅解析实际主题（深→tokyonight / 浅→blue）
    _theme = resolve_theme(_theme, bool(ctx.cfg.get("follow_system")))
    ...
    # 跟随系统深浅色：系统主题切换（Win 深浅色开关）时实时重应用
    def _reapply_theme(*_a):
        if not ctx.cfg.get("follow_system"):
            return
        t = resolve_theme(ctx.cfg.get("theme", "tokyonight"), True)
        w = ctx.cfg.get("wallpaper_path", "") or ""
        app.setStyleSheet(build_qss(
            t, w,
            card_opacity=ctx.cfg.get("card_opacity", 0.5),
            panel_opacity=ctx.cfg.get("panel_opacity", 0.5),
            font_scale=ctx.cfg.get("font_scale", "small")))
        win = getattr(ctx, "win", None)
        if win is not None:
            win.apply_backdrop(t, w)
